...
int main(int argc, char *argv[])
{
  ...
  NBodyEnv::System testSystem(NBodyEnv::Functions::getGravFunc(),
                              NBodyEnv::EulerDiscretizer(), 1.0, {50000.0, 50000.0, 50000.0}, {-50000.0, -50000.0, -50000.0});

  NBodyEnv::Particle particleOne(NBodyEnv::gravitational, {2000.0, 400.0, 100.0},
  ...
  NBodyEnv::Particle particleThree(NBodyEnv::gravitational, {2000.0, 700.0, 1500.0},
                                   {5e-3, 0.0, 0.0}, 1.0e10, 50);
  NBodyEnv::Particle particleFour(NBodyEnv::gravitational, {2000.0, 500.0, 2500.0},
                                  {5e-3, 0.0, 0.0}, 1.0e10, 50);
  NBodyEnv::Particle particleFive(NBodyEnv::gravitational, {2000.0, 700.0, 5000.0},
                                  {5e-3, 0.0, 0.0}, 1.0e10, 50);
  NBodyEnv::Particle particleSix(NBodyEnv::gravitational, {5000.0, 1000.0, 5000.0},
                                 {-5e-3, 0.0, 0.0}, 1.0e10, 50);
  NBodyEnv::Particle particleSeven(NBodyEnv::gravitational, {-5000.0, -900.0, -5000.0},
                                   {-5e-3, 0.0, 0.0}, 1.0e10, 50);
  ...
  // Create exporter
  NBodyEnv::Exporter exporter("../../../graphics/testBH08EULER.part", 1.0);

  // Create Simulator
  NBodyEnv::Simulator<NBodyEnv::EulerDiscretizer> simulator(
      testSystem, &exporter, 50000, 100);
  simulator.runBH();

  exporter.close();

   // Create exporter
  NBodyEnv::Exporter exporter2("../../../graphics/testEULER.part", 1.0);

  // Create Simulator
  NBodyEnv::Simulator<NBodyEnv::EulerDiscretizer> simulator2(
      testSystem, &exporter2, 50000, 100);
  simulator2.run();
  ...
  //   std::cout << "Force on particle: " << force[0] << " " << force[1] << force[2] << std::endl;

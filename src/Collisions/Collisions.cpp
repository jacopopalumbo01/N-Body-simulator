...
namespace NBodyEnv
{
    void Collisions::elasticCollision(Particle &p1, Particle &p2)
    {
        ...
        std::cout << "Elastic collision at: " << p1.getPos().xPos << ", " << p1.getPos().yPos << ", " << p1.getPos().zPos << std::endl;
        // print particles velocities
        // std::cout << "P1 velocity: " << p1.getVel().xVel << ", " << p1.getVel().yVel << ", " << p1.getVel().zVel << std::endl;
        // std::cout << "P2 velocity: " << p2.getVel().xVel << ", " << p2.getVel().yVel << ", " << p2.getVel().zVel << std::endl;
    ...
    {   
        // NOTE: here we consider a PERFECTLY INELASTIC COLLISION, so the two particles will merge into one 
        // with the sum of their masses and the velocity of the new particle will be the average of the two
        // print collision position
        std::cout << "Inelastic collision at: " << p1.getPos().xPos << ", " << p1.getPos().yPos << ", " << p1.getPos().zPos << std::endl;
        ...
        // set new velocity and mass for the first particle, which will be the merged one
        p1.setSpecInfo(totMass);
        p1.setVel({xVel, yVel, zVel});

        // compute mass density of both particles ==> d = m/V = m / (4/3 * pi * r^3)
        double massDensityP1 = p1.getSpecInfo() / (4.0 / 3.0 * M_PI * p1.getRadius() * p1.getRadius() * p1.getRadius());
        double massDensityP2 = p2.getSpecInfo() / (4.0 / 3.0 * M_PI * p2.getRadius() * p2.getRadius() * p2.getRadius());
        ...
        // compute new radius and set it by reverting the previous formula ==> r = (3/4 * pi * m / d)^(1/3)
        double newRadius = pow(3.0 / (4.0 * M_PI) * totMass / massDensityMerged, 1.0 / 3.0);
        p1.setRadius(newRadius);

        // TODO: before being able to use this, we need to modify the System class to be able to remove particles
    }

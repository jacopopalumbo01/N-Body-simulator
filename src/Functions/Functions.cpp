...
namespace NBodyEnv
{
  void Functions::getGrav(Particle &p1, Particle &p2)
  {
    ...
    // if a particle goes out of the domain, it will simulate an elastic collision ==> just need to invert the velocity component
    // along the axis where the particle has gone out of the domain
    // TODO: move this somewhere else, since the new velocity values are gonna be overwritten by the System::compute() function
    // when it calls iter->updateVel(_deltaTime)
    // if(p1.getPos().xPos > 1.0 || p1.getPos().xPos < -1.0)
    // {
